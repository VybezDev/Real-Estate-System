...
<html lang="en">
  ...
  <body>
		...
		else  //user is logged in.
		{
			...
			if ((isset($_POST['decline']) || isset($_POST['accept'])) && ($_SESSION['currentOfferStatus'] == 'Decline' || $_SESSION['currentOfferStatus'] == 'Accept'))
			...
			else if (isset($_POST['decline']))
			{
				...
				if (mysqli_query($conn,$updateQuery))
					$message = 'Offer was successfully declined.';
			}
			else if (isset($_POST['accept']))
			{
				...
				if (mysqli_query($conn,$updateQuery))
				{
					...
					$message = 'Offer was successfully accepted.';
				}
				else 
			...
			//display the navbar and check specific property offers info.
			display_userNavbar();
			 ?>
				<div class='container'>
				<table>
					<tr>
						...
						<td> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; </td>
						<td><?php display_checkPropertyOffers_form(); ?></td>
					</tr>
				</table>
				<div> <?php
...
<?php
function display_checkPropertyOffers_form()  //allow agents to see all offers on current property.
{
	$currentProperty = $_SESSION['currentAgentViewProperty'];
	
	$conn = db_connect();	//connect to database
	
	//query to see if an offer was accepted, which will be used for sorting the offers list.
	$query_acceptance = "select * from offers where agentUsername like '".$_SESSION['valid_user']."' and propertyID = '".$currentProperty."' and offerStatus = 'Accept'";
	$result_acceptance = mysqli_query($conn,$query_acceptance);
	$num_acceptanceResults = mysqli_num_rows($result_acceptance);
	
	if ($num_acceptanceResults > 0)	//if there is an accepted offer, sort by accepted offer first.
		$query = "select * from offers where agentUsername like '".$_SESSION['valid_user']."' and propertyID = '".$currentProperty."' order by offerStatus";
	else 	//if no accepted offer, then show pending offers first.
		$query = "select * from offers where agentUsername like '".$_SESSION['valid_user']."' and propertyID = '".$currentProperty."' order by offerStatus desc";
	...
	{
		//no pending offers, and set values to blank.
	...
	$currentURL = isset($_GET['view']) ? "propertyOffers.php?view=".$_GET['view'] : "propertyOffers.php";
	
	echo "<form method='post' action=".$currentURL.">";
	?>
	<table bgcolor="#99CC99">
		<tr>
			<td colspan="2"><h3>Check Property Offers:</h3></td>
		</tr>
		<tr>
		...
		<tr>	
			<td height="50" colspan="2" align="center"><input type="submit" value="Decline" name="decline"/></td>
	...
	echo "<p><a href=propertyOffers.php?view=".$prevPage.">Previous Offer</a>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<a href=propertyOffers.php?view=".$nextPage.">Next Offer</a></p>";
